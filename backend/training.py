...
# ---------- 保存 ----------
# 白名单：表名 → 允许用于「删除缺失行」的 ID 列名。
# _delete_missing 用 f-string 拼 SQL，调用点目前全是字面量，但拼 SQL 的接口
# 迟早会被人传变量进来；先在这里锁死可删的表与列，传别的直接拒绝执行。
_DELETE_WHITELIST = {
    "training_projects": "project_key",
    "training_sessions": "session_id",
    "training_rounds": "round_id",
    "training_attempts": "attempt_id",
}
...
def _delete_missing(conn, table, col, keep):
    ...
    """
    if _DELETE_WHITELIST.get(table) != col:
        print(f"[training._delete_missing] 拒绝执行：{table}.{col} 不在白名单内")
        return
    try:
        if keep:
            ph = ",".join(["?"] * len(keep))
