...
def _resolve_stage(text_stage, forced_stage):
    ...
    # 兜底 = 当前进度所在阶段。注意 stage=0（第一阶段）是合法值，
    # 不能写 `stage or 1` —— 那会把 0 当成空值强行换成 1，
    # 用户在第一阶段导入「第N周」时进度被拽到 stage=1，
    # 周报按新 stage 统计，之前 stage=0 的学习数据全部"消失"（周报没数据的根因之一）。
    try:
        st = (svc.get_progress() or {}).get("stage")
        return int(st) if st is not None else 1
    except Exception:
        return 1
