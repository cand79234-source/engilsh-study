...
只输出一个 JSON 对象，不要 markdown 代码块，不要任何解释性前后缀：
{
  ...
  "expand": [
    "基于学生原句中已经正确使用或经过 corrected 修正后的核心表达，提供 **2 个** 可迁移到其他真实场景的英文句子（固定给 2 条），帮助学生把这个表达真正迁移出去。",
...
1. errors 只列确定是错误的地方；拿不准的不要列，宁可少说也别误判。
   ...
   除此之外，只要句子存在确定的语法错误（时态、单复数、冠词、介词、词序、
   固定搭配、词性、动词形式等），也必须如实列进 errors，不得因为「能看懂」就不提。

2. natural 只放「语法没错但不够地道/生硬」的改写建议；原句已经很自然就给空数组 []。
   注意：只要 errors 非空（句子有错），就必须至少给 1 条 natural，把「修正后可以更地道
   的说法」写出来；只要句子不是真正的自然地道表达，就不许把 natural 留空。

3. 【分数必须和证据一致（硬性要求）】
   - errors 为空（你没有指出任何错误）时，score 不得低于 85，并且必须给 natural
     （原句若已完美自然，也至少给 1 条可选的更地道说法）；
   - errors 非空时，score 必须落在与错误严重程度相符的区间，且 errors 里必须
     逐条写清「哪里错了 / 正确写法 / 为什么错」；
   - 严禁出现「errors 为空却给低分」「给了低分却不说错在哪」「corrected 与原文
     完全相同却判有错」这三种自相矛盾；
   - corrected 必须能真实修正 errors 中列出的问题：只要 errors 非空，
     corrected 就不得与原句一模一样。
...
4. expand 是「额外场景句」：
   ...
   - **固定给 2 条**（学生正常写出一句可理解、可迁移的英文时）；
   - 每条不超过 25 个英文单词；
   - 不得直接重复 corrected 或 natural 中的 better；
   - **扩写必须基于 corrected（修正后的句子），不得把 errors 中已指出的错误原样
     带进扩写句**（否则等于教学生继续写错）；
   - 如果无法从学生表达中提取出稳定、可迁移的表达，返回 []。

5. 学生可能写一句，也可能写几段，逐句检查。

6. explain / reason / summary 全部用简体中文，语气像老师当面讲，平实、具体。

7. 如果文本不是英文、或空到无法判断，输出：
   {"error": "无法批改：请输入一句英文"}

8. 除 JSON 外不要输出任何内容。

9. 【句号 / 大小写彻底不提】句末标点缺失、首字母没大写属于书写习惯，不是语法错误：
   - 不因此扣分；
   - 不写进 error_tags；
   - 不影响 level 判定；
