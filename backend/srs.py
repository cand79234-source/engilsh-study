...
from db import get_conn, ts, app_now, app_today


def _due_date(interval_days):
    return (app_today() + timedelta(days=int(interval_days))).isoformat()
...
def submit_review(review_id, correct, quality=None):
    ...
    conn.execute(
        ...
        (ease, interval, reps, next_due, last_score, total_correct, total_wrong,
         app_now().isoformat(timespec="seconds"), review_id))
    conn.commit()
    result = {
...
def update_output_star(word, status, score=0, stage=None, week=None):
    """③ 造句五星：更新一个词的「主动输出熟练度」。
    ...
    stage/week：当前课程周。首次记录（或老行缺省）时补写到 word_output，
    否则周报「新学词汇」按 stage+week 统计永远查不到（恒 0）——数据断链修复。
    """
    if not word:
        return None
    w = word.strip().lower()
    ...
    if row:
        ...
        conn.execute(
            ...
            " last_score=?, last_at=?, updated_at=?,"
            " stage=COALESCE(stage,?), week=COALESCE(week,?) WHERE word=?",
            (stars, (row["total_attempts"] or 0) + 1, status,
             int(score or 0), now, now, stage, week, w))
    else:
        ...
        conn.execute(
            ...
            " VALUES (?,?,?,?,?,?,?,?,?,?)",
            (w, stars, 1, status, int(score or 0), now, now, now, stage, week))
    conn.commit()
    conn.close()
    return stars
...
def stars_map(words):
    ...
    if not words:
        return out
    keys = sorted(set((x or "").strip().lower() for x in words if x and x.strip()))
    if not keys:
        return out
    conn = get_conn()
    try:
        # 原来是一个词一条 SELECT（N+1），词表上百个词就上百次往返。
        # 改成一次 IN 批量取；分片是为了不撞 SQLite 老版本的 999 参数上限。
        for i in range(0, len(keys), 500):
            chunk = keys[i:i + 500]
            ph = ",".join("?" for _ in chunk)
            for row in conn.execute(
                    "SELECT word, stars FROM word_output WHERE word IN (%s)" % ph,
                    tuple(chunk)).fetchall():
                out[row["word"]] = row["stars"]
                out.setdefault((row["word"] or "").lower(), row["stars"])
    finally:
        conn.close()
    return out
...
def star_recycle_due(words, days=14):
    ...
    cutoff = (app_now() - timedelta(days=int(days))).isoformat()
    conn = get_conn()
