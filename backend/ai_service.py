...
def days_ago_str(n):
    ...
    return (app_now() - timedelta(days=n)).isoformat(timespec="seconds")
...
# 题目要写「现在 / 每天都做的事」的常见说法
_REQ_HABIT = ("每天", "每日", "习惯", "日常", "固定安排", "正在做", "此刻",
              "routine", "habit", "daily")
...
# =====================================================================
# 新增：① want/need/decide + 动词原形 → 缺 to
#       ② 整句常见拼写错误（词表精确匹配，不做模糊猜测以避误报）
#       ③ more / many / 数词 + 单数可数名词 → 用复数
# ---------------------------------------------------------------------
# 背景：以前 "we usually need finsh more assignment" 判 95 分「没有明显错误」，
#       因为 _r_verb_pattern 只查「head + doing → to do」，查不到
#       「head + 动词原形」，而 finsh 的拼写也不在目标词检查范围内。
# =====================================================================

# 后面必须接 to do（to + 动词原形）的动词；直接跟原形就是缺 to
_TO_INF_HEAD = (
    r"need|needs|needed|want|wants|wanted|decide|decides|decided|"
    r"hope|hopes|hoped|plan|plans|planned|promise|promises|promised|"
    r"agree|agrees|agreed|choose|chooses|expect|expects|expected|"
    r"learn|learns|try|tries|tried|prepare|prepares|prepared|"
    r"refuse|refuses|afford|affords|offer|offers|pretend|pretends|"
    r"wish|wishes|intend|intends|seem|seems|would\s+like|would\s+love"
)

# 这些词在 need/want 后面通常是**名词**用法，不能判成「缺 to」
_INF_HEAD_NOUN_OK = {
    "help", "practice", "support", "rest", "sleep", "care", "love", "change",
    "training", "exercise", "advice", "water", "time", "money", "food",
    "attention", "courage", "energy", "patience", "luck", "space", "room",
    "something", "anything", "everything", "nothing", "way", "reason",
}

# 高频拼写错误（错 → 对）。只收常见词，精确匹配，不做模糊推断。
_COMMON_TYPOS = {
    "finsh": "finish", "finesh": "finish", "finsih": "finish",
    "wrok": "work", "worck": "work", "wirte": "write", "writting": "writing",
    "studing": "studying", "studyed": "studied", "studyied": "studied",
    "becuase": "because", "becasue": "because", "bcoz": "because",
    "recieve": "receive", "seperate": "separate", "definately": "definitely",
    "occured": "occurred", "untill": "until", "wich": "which", "teh": "the",
    "thier": "their", "freind": "friend", "sucess": "success",
    "succesful": "successful", "neccessary": "necessary", "adress": "address",
    "begining": "beginning", "beleive": "believe", "calender": "calendar",
    "enviroment": "environment", "goverment": "government",
    "grammer": "grammar", "happend": "happened", "independant": "independent",
    "knowlege": "knowledge", "managament": "management", "peice": "piece",
    "recomend": "recommend", "refered": "referred", "relevent": "relevant",
    "sence": "since", "tommorow": "tomorrow", "tommorrow": "tomorrow",
    "truely": "truly", "unfortunatly": "unfortunately", "alot": "a lot",
    "improtant": "important", "importent": "important", "importamt": "important",
    "diffrent": "different", "diffcult": "difficult", "dificult": "difficult",
    "confortable": "comfortable", "comfertable": "comfortable",
    "expierence": "experience", "experiance": "experience",
    "langauge": "language", "languge": "language", "vocabulery": "vocabulary",
    "pronounciation": "pronunciation", "restarant": "restaurant",
    "restaurent": "restaurant", "buisness": "business", "busness": "business",
    "carrer": "career", "developement": "development", "maneger": "manager",
    "managment": "management", "oppertunity": "opportunity",
    "oportunity": "opportunity", "proffesional": "professional",
    "profesional": "professional", "shedule": "schedule",
    "schedual": "schedule", "scedule": "schedule", "trainning": "training",
    "homewrok": "homework", "classrom": "classroom", "tommorrow": "tomorrow",
    "exercize": "exercise", "familar": "familiar", "mispell": "misspell",
    "noticable": "noticeable", "occassion": "occasion", "priviledge": "privilege",
    "publically": "publicly", "speach": "speech", "succeded": "succeeded",
    "tommorow": "tomorrow", "tounge": "tongue", "usefull": "useful",
    "usefulness": "usefulness", "wether": "whether", "whith": "with",
    "wroking": "working", "workin": "working", "goin": "going",
    "everydat": "everyday", "everday": "everyday", "tomorow": "tomorrow",
    "mornin": "morning", "evenin": "evening", "weeked": "weekend",
    "favourate": "favourite", "favorate": "favorite", "realy": "really",
    "usualy": "usually", "usally": "usually", "alwasy": "always",
    "alway": "always", "oftem": "often", "sometimes": "sometimes",
    "importamt": "important", "contry": "country", "countery": "country",
    "colluege": "colleague", "collegue": "colleague", "assigment": "assignment",
    "asignment": "assignment", "assignement": "assignment",
    "deadlin": "deadline", "schedul": "schedule", "meetin": "meeting",
    "reporte": "report", "emial": "email", "companie": "company",
    "compnay": "company", "conpany": "company", "ofice": "office",
    "offcie": "office", "maner": "manner", "problam": "problem",
    "probelm": "problem", "answear": "answer", "quetion": "question",
    "qustion": "question", "englis": "english", "inglish": "english",
    "langauge": "language", "practiceing": "practicing",
}


def _typo_fix(t):
    """把一个 token 纠正成它在词典里的样子；认不出来返回 None。"""
    return _COMMON_TYPOS.get(t)


def _is_known_form(t):
    """t 是不是某个已知动词的合法变形（三单/过去/-ing/-ly）。"""
    if t in VERBS or t in _AUX_BE or t in _MODALS:
        return True
    for b, (s3, past) in _IRREGULAR.items():
        if t in (s3, past):
            return True
    if t.endswith("ies") and t[:-3] + "y" in VERBS:
        return True
    if t.endswith("ied") and t[:-3] + "y" in VERBS:
        return True
    if t.endswith("s") and t[:-1] in VERBS:
        return True
    if t.endswith("ed") and (t[:-1] in VERBS or t[:-2] in VERBS):
        return True
    if t.endswith("ing"):
        stem = t[:-3]
        if (stem in VERBS or stem + "e" in VERBS
                or (len(stem) > 1 and stem[-1] == stem[-2] and stem[:-1] in VERBS)):
            return True
    if t.endswith("ly") and (t[:-2] in VERBS or t[:-2] + "e" in VERBS):
        return True
    return False


def _r_infinitive_to(s, low):
    """need / want / decide 等后面直接跟动词原形 → 中间缺 to。

    例：we need finsh it → we need to finish it
    · 后面的词是常见名词（help / time / money …）时不报，避免 "I need help" 误伤；
    · 后面的词拼错了（finsh）也认：先按拼写表纠正，确认它是动词再报。
    """
    out = []
    for m in re.finditer(rf"\b({_TO_INF_HEAD})\s+([a-z]+)\b", low):
        head, nxt = m.group(1), m.group(2)
        if nxt in _INF_HEAD_NOUN_OK or nxt in _AUX_BE or nxt in _MODALS:
            continue
        verb = nxt if nxt in VERBS else _typo_fix(nxt)
        if not verb or verb not in VERBS or verb.endswith("ing"):
            continue
        where = f"{head} {verb}"
        fixed = f"{head} to {verb}"
        out.append(_err(
            "固定搭配", where, fixed,
            f"{head} 后面接动词时要用 **to do**（带 to 的不定式），"
            f"不能直接跟动词原形。应写成 {fixed}。",
            m.span(1), f"{head} to", "medium"))
    return out


def _r_word_spelling(raw, low, word=""):
    """整句拼写检查：命中常见错词表才报，不做模糊猜测（避免误伤正常词）。"""
    out = []
    w = (word or "").strip().lower()
    for m in re.finditer(r"[a-z]+", low):
        t = m.group(0)
        if len(t) < 4 or t in _COMMON_WORDS or t in _AUX_BE or t in _MODALS:
            continue
        if t in VERBS or _is_known_form(t):
            continue
        # 目标词及其近似写法交给 _r_target_spelling，这里不重复报
        if w and (t == w or (len(w) >= 4 and _edit_dist(t, w) <= 1)):
            continue
        fix = _typo_fix(t)
        if not fix or fix == t:
            continue
        out.append(_err(
            "拼写", t, fix,
            f"「{t}」拼写有误，正确写法是 **{fix}**。",
            (m.start(), m.end()), fix, "medium"))
    return out


_UNCOUNTABLE = {
    "water", "time", "money", "music", "homework", "work", "information",
    "advice", "news", "weather", "traffic", "furniture", "luggage", "food",
    "rice", "tea", "coffee", "sugar", "salt", "air", "help", "sleep",
    "research", "knowledge", "english", "chinese", "progress", "equipment",
    "staff", "cash", "energy", "health", "practice", "fun", "space", "love",
}
_COUNT_HINT = (r"more|many|several|(?:a\s+)?few|two|three|four|five|six|"
               r"seven|eight|nine|ten|twelve|twenty")


def _r_more_plural(raw, low, word=""):
    """more / many / 数词 + 单数可数名词 → 复数。

    只作为**优化建议**提示，不算错误、不扣分（"more + 单数" 有时是抽象用法，
    判成错会冤枉人）。由 _optimizations 调用。
    """
    out = []
    w = (word or "").strip().lower()
...
def _task_issues(raw, low, word, task_grammar, task_prompt):
    """题目要求提醒（不进错题本、不压分、不阻止 PASS，只在最后的小框里提一嘴）。

    三条：① 该写「现在 / 每天做的事」却读不出频率 ② 时态不贴题 ③ 没用上目标词。
    文案必须短 —— 展示区只有一个小框，最长三行。
    """
    issues = []
    req = " ".join([task_grammar or "", task_prompt or ""]).lower()

    # ① 题目要写「现在 / 每天都做的事」→ 得读出频率、时间，或用现在进行时
    if any(k in req for k in _REQ_HABIT) or any(k in req for k in _REQ_FREQ):
        toks = low.split()
        has_freq = any(f in toks for f in _FREQ_ADV) or bool(_FREQ_RE.search(low))
        cont = re.search(r"\b(?:am|is|are|'m|'s|'re)\s+[a-z]+ing\b", low)
        if not has_freq and not cont:
            issues.append({
                "type": "任务要求", "where": "（习惯/频率）", "correct": "",
                "explanation": "这题要写现在或每天做的事，加个 usually / often / "
                               "every day 会更贴题。",
                "severity": "medium"})

    # ② 一般现在时 vs 过去时间词（只是提醒，不判错）
    if any(k in req for k in _REQ_PRESENT):
        if _PAST_MARKER.search(low):
            issues.append({
                "type": "任务要求", "where": "（时态）", "correct": "",
                "explanation": "题目练的是一般现在时，这句出现了过去时间词"
                               "（yesterday / ago / last …）。",
                "severity": "medium"})
...
RULES = [
    ...
    _r_infinitive_to,    # need finish -> need to finish
...
def _dedupe(errors):
    ...
    拼写优先：拼错的动词（recieve）会让「整句缺谓语」这类大区间错误先被接受，
    把真正的拼写问题吞掉，所以拼写类先排、先占位。
    """
    _PRIO = {"拼写": 0, "其他": 0}
    accepted = []
    for e in sorted(errors, key=lambda x: (_PRIO.get(x["type"], 1),
                                           x["span"][0],
                                           -(x["span"][1] - x["span"][0]))):
        a, b = e["span"]
...
# 扩写用的「原因尾巴」：按句子里出现的主题词挑，避免每句都套同一个空话
_TAIL_BY_TOPIC = [
    (("company", "work", "job", "meeting", "deadline", "plan", "report",
      "project", "client", "email", "office", "schedule", "manager", "team",
      "colleague", "boss", "task"),
     ("my manager asks me to", "my manager asked me to")),
    (("study", "english", "school", "class", "book", "learn", "course",
      "exam", "teacher", "student", "word", "vocabulary", "practice"),
     ("I want to improve my English", "I wanted to improve my English")),
    (("trip", "travel", "hotel", "flight", "airport", "ticket", "luggage",
      "passport", "tour", "visa"),
     ("I am preparing for a trip", "I was preparing for a trip")),
    (("food", "eat", "lunch", "dinner", "cook", "restaurant", "coffee",
      "tea", "breakfast"),
     ("it is part of my daily routine", "it was part of my daily routine")),
    (("health", "exercise", "run", "gym", "sleep", "doctor", "hospital"),
     ("it keeps me healthy", "it kept me healthy")),
]
_TAIL_DEFAULT = ("it is part of my daily routine", "it was part of my daily routine")
...
def _expand_samples(s, low, past=False):
    ...
    for sample, note in pool:
        ...
        if sample in seen:
            continue
        seen.add(sample)
        out.append((sample, note))
        if len(out) >= 2:
            break
    return out


def _optimizations(s, low, req="", word=""):
    """参考建议（仅供参考，不影响判定与分数；**任何分数都会给**）。

    返回两类：
      - 润色类：{where, suggestion, reason} —— 前端按「建议」展示
      - 扩写类：{kind:'expand', sample, note} —— 前端按「扩写：」展示，sample 是完整句子
    两类都只影响展示，不进错误本、不扣分。
    """
    opts = []
    if s and s[-1] not in ".!?":
    ...
    # 题目点名要练频率副词、但句子里没有 → 只建议，不再判「未完成」
    # 两条前提：① 句子够短还有地方加（够长的句子不再硬塞）
    #           ② 题目不是「写过去的事」——昨天做的事再加 every day 就自相矛盾了
    req_low = (req or "").lower()
    past_task = any(k in req_low for k in ("过去时", "昨天", "上周", "昨晚",
                                           "以前", "past", "yesterday"))
    if any(k in req_low for k in _REQ_FREQ) and not past_task \
            and len(re.findall(r"[A-Za-z']+", s)) < 12:
        toks = low.split()
        has_freq = any(f in toks for f in _FREQ_ADV) or bool(_FREQ_RE.search(low))
        if not has_freq:
            opts.append({
                "where": "频率",
                "suggestion": (s.rstrip().rstrip(".!?") + " every day."),
                "reason": "这周练的是频率副词，加一个 usually / often / every day "
                          "会更贴题；不加也不算错。",
            })
    # 可数名词被 more / many / 数词 修饰却用了单数 → 提示用复数（不算错）
    for where, fixed in _r_more_plural(s, low, word):
        opts.append({
            "where": where,
            "suggestion": s.replace(where, fixed, 1),
            "reason": f"{word} 是可数名词，说「不止一个」时用复数更地道：{fixed}。"
                      f"（写成单数也不算语法错误。）",
        })
...
def analyze(sentence, word="", task_grammar="", task_prompt=""):
    ...
    try:
        ...
        raw_errors.extend(_r_word_spelling(raw, low, word) or [])
    ...
    if errors:
        ...
        # 任务建议（没用上目标词 / 时态不贴题）只作为建议展示，
        # 不压分、不判「有错误」——语法没问题就该按正常分给。
        score = BASE_SCORE
    ...
    return {
        ...
        # 任何分数都给参考建议（以前只有 ok 才给，导致 84 分一片空白）
        # 建议基于「修正后的句子」来写，免得把错误句再扩写成更长的错句
        "optimizations": _optimizations(
            corrected or raw, _safe_lower(corrected or raw),
            " ".join([task_grammar or "", task_prompt or ""]), word),
    }


# =====================================================================
# 并发安全写入：sentence attempt 自增 / errors 归并
# ---------------------------------------------------------------------
# 旧写法是「先 SELECT 查最大值/查重 → 再 INSERT」，两个请求同时提交时会读到
# 同一个 attempt、或插出两条一模一样的错题。这里改成：
#   · SQLite：整个批改过程放进 BEGIN IMMEDIATE（立刻拿写锁），写操作串行化；
#   · 唯一索引 ux_sentences_attempt / ux_errors_merge 兜底；
#   · 万一仍撞车（PG 无写锁串行化），回滚后重新取值重试，绝不产生重复。
# 历史数据、attempt 语义、错误分类逻辑全部不变。
# =====================================================================
...
def _lock_task_attempt(conn, stage, week, day, task_key):
    """PG 下把「同一道题的 attempt 分配」排成队列。

    SQLite 靠 BEGIN IMMEDIATE 的库级写锁就够了；PG 是 MVCC，两个事务会同时
    读到同一个 MAX(attempt) 并算出同一个新值，只靠「撞唯一索引再重试」在
    并发高时会连续撞车、把重试次数耗光。这里用**事务级**咨询锁（commit /
    rollback 时自动释放，不会泄漏），把分配动作真正串行起来。

    拿不到锁（老版本 PG 等）就静默退回冲突重试，绝不阻断批改。
    """
    if not _using_pg():
        return
    try:
        import zlib
        k1 = int(stage) * 10000 + int(week) * 100 + int(day)
        k2 = int(zlib.crc32((task_key or "").encode("utf-8")) & 0x7FFFFFFF)
...
def _insert_sentence_with_attempt(conn, stage, week, day, word, task_key, res, now):
    ...
    for _ in range(_ATTEMPT_RETRY):
        try:
            ...
            # 重试时别再撞同一个号：PG 的快照可能还是旧的（别人未提交），
            # 这里强制往后顺延，保证每次重试都往前推进一格。
            if attempt <= tried:
                attempt = tried + 1
            tried = attempt
            sid = insert_get_id(
...
def _bump_error(conn, eid, sentence_text, now):
    """已存在的错题：次数 +1，刷新最后出现时间与句子。"""
    conn.execute(
        "UPDATE errors SET times=COALESCE(times,0)+1, last_at=?, sentence_text=?"
        " WHERE id=?", (now, sentence_text, eid))


def _merge_error(conn, word, etype, norm, where, e, task_key, original, now):
    """归并一条错题本记录并返回 errors.id（存在则累加，不存在则新建）。"""
    sel_sql = ("SELECT id FROM errors WHERE source='sentence' AND word=?"
               " AND error_type=? AND norm_text=?")
    sel_args = (word, etype, norm)

    row = conn.execute(sel_sql, sel_args).fetchone()
    if row:
        eid = int(row["id"])
        _bump_error(conn, eid, original, now)
        return eid

    pg = _using_pg()
    if pg:
        # PG 事务一旦报错就整个失效，用 SAVEPOINT 把「试探性插入」隔离出来
        conn.execute("SAVEPOINT eos_err_ins")
    try:
        eid = insert_get_id(
            conn,
            ...
            (etype, where, e["correct"], e["explanation"], "sentence", now,
             word, task_key, where, norm, LEVEL_MEMORY, original,
             1, now, now, 0))
        if pg:
            conn.execute("RELEASE SAVEPOINT eos_err_ins")
        return eid
    except Exception:
        # 唯一索引 ux_errors_merge 拦住了并发重复插入
        if pg:
            for stmt in ("ROLLBACK TO SAVEPOINT eos_err_ins",
                         "RELEASE SAVEPOINT eos_err_ins"):
                try:
                    conn.execute(stmt)
                except Exception:
                    pass
...
def correct_sentence(sentence, stage=0, week=3, day=1, word="", task_key="",
    ...
    try:
        _begin_write(conn)
        # PG 下先把这道题的 attempt 分配串行化（SQLite 已由 BEGIN IMMEDIATE 保证）
        ...
        for e in res["errors"]:
            ...
            bank_id = _merge_error(conn, word, etype, norm, where, e, task_key,
                                   res["original"], now)
