...
4. 不写编号事件清单：不要把十几二十件小事按 1. 2. 3. 编号拼成一个"场景"。
...
- large 大情景：一个更完整、更丰富的真实情境，能**自然容纳多个学习词**，
  有因果、有时间线、有连续性，是一个说得通的整体。
  例：刚入职公司的第一周——收拾工位、认识同事、参加入职培训、跟经理过一遍在跟的项目，
  里面 account / prepare / project / colleague / training / manager 都能自然出现。
  大情景不是把十几个小事件硬拼在一起，也不是按编号报流水账。
  （大情景可能还要带上几个复习词，规则见下。）
