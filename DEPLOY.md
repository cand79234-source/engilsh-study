...
## 🤖 AI 三件套（2026-09-09 新增，都走同一个 ARK_API_KEY）

三处 AI（情景生成 / 批改 AI 出分 / 薄弱项讲解）共用 Render 上已声明的两个环境变量：

| 环境变量 | 在哪填 | 说明 |
|---|---|---|
| `ARK_API_KEY` | Render 控制台 Environment | 火山方舟 Key，只填在这里，绝不进代码仓库 |
| `ARK_MODEL` | Render 控制台（可留空） | 默认 `doubao-seed-2-0-mini-260428` |

**不填 Key 会怎样**：情景区整块隐藏、批改回落本地规则（分数旁标「本地估算，仅供参考」）、薄弱项讲解不触发。其它功能完全不受影响。

**填了 Key 之后**：

| 功能 | 触发点 | 你能感知到的变化 |
|---|---|---|
| 情景生成 | 导入整周词时后台批量生成（3-5 条/词）；点「记住了」时兜底补生成 | 打开 `/stealth` 造句前，先看到一条中文生活情景；点 🔁 换一条；>90 分会提示「换下一个场景？」 |
| 批改 AI 出分 | 造句时开「AI」开关 | 显示 AI 给的分数和判定（基本掌握/需要改进），"communicate very important base work" 这种不再给 90 分；句号/大小写不再提 |
| 薄弱项讲解 | 同一个词的同一个错反复 ≥3 次 | 批改结果区原地多一行「⭐ 你为啥总在这栽跟头」，同一个错只讲一次 |

**验收**：`python3 tests/test_scenario_weakness.py` 和 `python3 tests/test_stealth_ai_flow.py`（本地跑，用假 AI 响应，不花 token）。

**摸鱼模式**：独立 `desktop/` Tauri 壳，在本机编译（见 `desktop/README.md`），连 Render 后端，与本体零耦合。
